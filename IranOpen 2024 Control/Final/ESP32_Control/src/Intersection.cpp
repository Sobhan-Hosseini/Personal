...
void Intersection::intersection_preparation(){
    ...
    if(motion.go_for_distance(400 - preparation_value, motion.go_straight(8.1), motion.obstacle_controller(20))){ //edit: a:    400
...
void Intersection::angle_correction(){
    ...
    int steer = 0;
    correction = 1.4 * crosswalk_angle;
    if(correction > 0){
        steer = 90;
    }
    else{
...
void Intersection::straight_II(){
    if(line_direction_parameter == 0){
        ...
        if(motion.go_for_distance(350, 0, motion.obstacle_controller(25))){ //edit:    a: 
            motion.stop();
            vTaskDelay(500);
            line_direction_parameter = vision.get_line_direction();
        }
...
// uart.debug_println("right_I: ");
    if(motion.go_for_angle(50, -85, motion.obstacle_controller(40))){   //-90
        motion.stop();
        vTaskDelay(500);
        motor.start_encode(); 
        current_instruction = &Intersection::right_II;
...
void Intersection::right_III(){
    ...
    if(motion.go_for_angle(35, -85, motion.obstacle_controller(40))){
        motion.stop();
        vTaskDelay(500);
        motor.start_encode(); 
        current_instruction = &Intersection::side_correction;
...
void Intersection::left_I(){    //edit:    g:
    if(motion.go_for_distance(175 + crosswalk_angle, 0, motion.obstacle_controller(20))){  //edit : angle correction shift
        motion.stop();
        vTaskDelay(500);
        motor.start_encode();
